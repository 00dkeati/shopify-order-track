// IMPROVED API - Finds orders and checks for portrait delivery via timeline
export default async function handler(req, res) {
  ...
  try {
    ...
    // Method 2: Search by order name without #
    if (!order) {
      try {
    ...
    // Method 3: Search all recent orders
    if (!order) {
      try {
    ...
    // Confirm it's a portrait order (optional check)
    ...
    // Fetch timeline events for portrait delivery
